	...
	t->assertNoException(
		[t]()
		{
			//
			{
				...
				std::cout << "Finished pop front and back, size is now " << std::to_string(list.size()) << std::endl;
...
void testIterators(Tests& tests)
{
	...
	t->assertNoException(
		[t]()
		{
			{
				//
				//std::cout << "Initializing list, itr, and size_t half for Iterator addition/subtraction operators test" << std::endl << std::endl;
				auto list = makeList();
				//std::cout << "Initialized list using makeList()" << std::endl;
				auto itr = list.begin();
				//std::cout << "Initialized itr in cpp_tests, value of itr using list.begin() is " << std::to_string(itr.getCursor()->getElement()) << std::endl;
				//std::cout << "Initialized itr using list.begin(), itr is pointing to " << std::to_string(itr.getCursor()->getElement()) << std::endl;
				size_t half = list.size() / 2;
				//std::cout << "Initialized half using list.size() / 2, list size is " << std::to_string(list.size()) << " half is " << std::to_string(half) << std::endl;
				
				//
				//std::cout << "Itr + half is " << std::to_string(*(itr + half)) << std::endl;
				t->assertEqual(
					(*(itr + half)), static_cast<int>(0),
					1, "Addition operator"
				);
				//std::cout << "Itr + half - 3 is " << std::to_string(*((itr + half) - 3)) << std::endl;
				//list.Print();
				t->assertEqual(
		...
		{
			{
				...
				auto list = makeList();
				//std::cout << "Initial List Size is " << std::to_string(list.size()) << std::endl;
				//list.Print();
				auto itr_middle = list.begin() + list.size() / 2;
				//std::cout << "Itr_middle is " << std::to_string(itr_middle.getCursor()->getElement()) << std::endl;
		...
		{
			{
				...
				//std::cout << "Printing INSERT AFTER LIST AAAAAAAAAAAAAAAAAAAAAAAAAAAAAaa" << std::endl;
				//list.Print();
				//std::cout << "Now attempting to access list at " << std::to_string(idx) << " + 1" << std::endl;
				t->assertEqual(
		...
		{
			{
				...
				// Push after (middle)
				//This line results in infinite list AP123
...
void testAssign(Tests& tests)
{
	...
	t->assertNoException(
		[t]()
		{
			//
			auto list1 = makeList();
			//std::cout << "List 1 size is " << std::to_string(list1.size()) << std::endl;
			//list1.Print();
			CPSC131::DoublyLinkedList::DoublyLinkedList<int> list2;
			//std::cout << "List 2 size is " << std::to_string(list2.size()) << std::endl;
			auto itr1 = list1.begin() + 5000000;	// lol no
			//std::cout << "itr1 is pointing to " << std::to_string(itr1.getCursor()->getElement());
			auto itr2 = itr1 + 100;
			list2.assign(itr1, itr2);
			//std::cout << "List 2 size is " << std::to_string(list2.size()) << std::endl;
	...
	t->assertNoException(
		[t]()
		{
			//
			{
				...
				//	and unit count
				//std::cout << "Store balance is " << std::to_string(store.getAccountBalance()) << std::endl;
